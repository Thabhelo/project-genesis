...
function App() {
  ...
  return (
    <div className="flex h-screen w-screen bg-[#09090b] text-[#f4f4f5] font-sans overflow-hidden text-[13px]">
      ...
      {/* MAIN LAYOUT - all panels resizable */}
      <div className="flex-1 flex flex-col min-w-0 bg-[#09090b] overflow-hidden">
        
        <header className="h-14 flex items-center justify-between px-6 shrink-0 border-b border-[#27272a]">
        ...
        <div className="flex-1 overflow-hidden flex flex-col min-h-0 px-4 pb-4">
          <Group orientation="horizontal" className="flex-1 min-h-0">
            {/* Left: Nav sidebar */}
            <Panel id="nav" defaultSize={12} minSize={8} maxSize={22} className="min-w-0 flex flex-col overflow-hidden">
              <div className="h-full flex flex-col p-3 bg-[#09090b] border-r border-[#27272a] overflow-hidden">
                <div className="flex items-center gap-2 px-2 mb-4 mt-1 shrink-0">
                  <img src="/logo.svg" alt="" className="w-6 h-6" />
                  <span className="font-medium text-[14px] text-white truncate">Project Genesis</span>
                </div>
                <button
                  onClick={() => {
                  ...
                  className="w-full bg-[#f4f4f5] text-[#09090b] hover:bg-[#e4e4e7] transition-colors py-1.5 rounded-md font-medium text-[13px] flex items-center justify-center gap-2 mb-6 shrink-0"
                >
                ...
                <div className="space-y-0.5 flex-1 overflow-y-auto min-h-0">
                  ...
                  <div className="mt-6 mb-2 px-3 text-[11px] font-medium text-[#71717a] uppercase tracking-wider">Governance</div>
                  ...
                  <div className="mt-6 mb-2 px-3 text-[11px] font-medium text-[#71717a] uppercase tracking-wider">World Domains</div>
                ...
                <div className="mt-auto pt-4 shrink-0">
                  <div className="px-3 mb-4">
                    ...
                    <div className="h-1 bg-[#27272a] rounded-full overflow-hidden">
                      <div className="h-full bg-[#f4f4f5] rounded-full transition-all duration-500" style={{ width: `${Math.min((resources / 1000) * 100, 100)}%` }} />
                    </div>
                  </div>
                  <div className="flex items-center gap-3 px-2 py-2 rounded-md mt-2">
                    {authReady ? (
                      user ? (
                        <>
                          <img src={user.photoURL || undefined} alt="" className="w-7 h-7 rounded-full bg-[#27272a] object-cover shrink-0" />
                          ...
                          <button onClick={signOut} className="p-1.5 rounded-md hover:bg-[#27272a] text-[#71717a] hover:text-[#f4f4f5] transition-colors shrink-0" title="Sign out">
                            <LogOut size={14} />
                          </button>
                        </>
                      ) : authLoading ? (
                      ...
                      <>
                        <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=Creator" alt="Creator" className="w-7 h-7 rounded-full bg-[#27272a] shrink-0" />
                        <div className="flex-1 min-w-0">
            ...
            <Separator className="shrink-0 bg-[#27272a] hover:bg-[#3f3f46] data-[resize-handle-state=drag]:bg-[#818cf8] transition-colors cursor-col-resize min-w-[8px] w-2" />
            {/* Agent cards */}
            <Panel id="agents" defaultSize={16} minSize={12} maxSize={28} className="min-w-0 pr-2 flex flex-col overflow-hidden">
              <div className="h-full overflow-y-auto flex flex-col gap-3 custom-scrollbar">
                {AGENTS.map(agent => {
                  ...
                  return (
                    <div key={agent.name} className="bg-[#09090b] border border-[#27272a] rounded-xl p-4 flex flex-col min-h-[110px] relative group hover:bg-[#18181b] transition-colors shrink-0">
                      <div className="flex flex-col items-center text-center gap-3">
                        <div className={`w-12 h-12 rounded-lg flex items-center justify-center shrink-0 bg-gradient-to-br ${agent.gradient}`}>
                          <span className="text-white text-xl font-medium">{GREEK_LETTERS[agent.name] ?? agent.name[0]}</span>
                        </div>
                        <div>
            ...
            {/* Center: Thinking + 3D World (vertical split) */}
            <Panel id="center" defaultSize={44} minSize={30} maxSize={65} className="min-w-0 flex flex-col overflow-hidden">
              <Group orientation="vertical" className="h-full min-h-0 flex-1 flex">
                <Panel id="thinking" defaultSize={45} minSize={15} maxSize={75} className="min-h-0 flex flex-col overflow-hidden">
                  <div className="h-full overflow-hidden">
                    {thinkingLogs.length === 0 ? (
                      <div className="h-full flex items-center justify-center rounded-xl border border-[#27272a] bg-[#09090b]">
                        <ThinkingBar
                ...
                <Separator className="shrink-0 bg-[#27272a] hover:bg-[#3f3f46] data-[resize-handle-state=drag]:bg-[#818cf8] transition-colors cursor-row-resize min-h-[8px] h-2" />

                <Panel id="world" defaultSize={55} minSize={25} maxSize={85} className="min-h-0 flex flex-col overflow-hidden">
                  <div className="h-full bg-[#09090b] border border-[#27272a] rounded-xl overflow-hidden flex flex-col">
                    <div className="p-4 border-b border-[#27272a] flex justify-between items-center bg-[#09090b] z-10 shrink-0">
            ...
            {/* Right: Tabs */}
            <Panel id="sidebar" defaultSize={28} minSize={18} maxSize={45} className="min-w-0 pl-2 flex flex-col overflow-hidden">
              <div className="h-full overflow-y-auto flex flex-col gap-6 custom-scrollbar">
                <TabContent
              activeTab={activeTab}
              ...
              <motion.div key={entry.timestamp ?? i} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="flex gap-3 items-start">
                ...
                <div className="flex-1 min-w-0 pt-0.5">
                  <p className="text-[13px] leading-snug">
                    ...
                    <span className="text-[#f4f4f5] block mt-0.5 break-words">&quot;{entry.message}&quot;</span>
                  </p>
                  ...
                  <div key={agent.name} className="border border-[#27272a] rounded-lg p-3">
                    ...
                    <div className="space-y-1.5 max-h-[120px] overflow-y-auto">
                      {entries.slice(-5).reverse().map((e: any, i: number) => (
                        <p key={i} className="text-[11px] text-[#a1a1aa] break-words">&quot;{e.message}&quot;</p>
                      ))}
                    </div>
                  </div>
                );
              ...
              {archive.length === 0 ? (
                ...
                archive.slice().reverse().map((a: any) => (
                  <div key={a.id} className="bg-[#18181b] rounded px-2 py-1.5 text-[11px]">
                    ...
                    <span className="text-[#a1a1aa] break-words block">{a.value}</span>
                  </div>
                ))
              )}
            </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {images.slice(-4).reverse().map((img: any) => (
                    <div key={img.id} className="rounded overflow-hidden border border-[#27272a] bg-[#18181b]">
                      ...
                      <p className="text-[10px] text-[#71717a] px-1 py-0.5 break-words" title={img.prompt}>{img.prompt}</p>
                    </div>
                  ))}
                </div>
              </>
